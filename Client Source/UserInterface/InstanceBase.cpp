...
BOOL CInstanceBase::isHoldingBouquet()
{
...
void CInstanceBase::SetAcceRefineEffect(BYTE acceDrainRate)
{
...
bool __IsTHWeapon(DWORD dwVnum)
{
	const std::vector<DWORD> vVnums = {
		330,
		350,
		1200,
		2220,
	...
	const auto& it = std::find_if(vVnums.begin(), vVnums.end(), [&dwVnum](DWORD thVnum) { return thVnum <= dwVnum && dwVnum <= (thVnum + 5); });
	if (it != vVnums.end())
		return true;
...
void CInstanceBase::RefreshAcceRefineEffect()
{
	if (m_acceRefineEffect)
	{
		__ClearAcceRefineEffect();
...
void CInstanceBase::SetShowMonsterEffects(bool show)
{
