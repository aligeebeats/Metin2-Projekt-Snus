...
class CPythonEventManager : public CSingleton<CPythonEventManager>
{
	...
	public:
		typedef struct SEventSet
		{
			...
			SEventSet() : ix(0), iy(0), iWidth(0), iyLocal(0), isLock(0), lLastDelayTime(0), iCurrentLetter(0), CurrentColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f)), strCurrentLine(""), pCurrentTextLine(nullptr), isConfirmWait(false), pConfirmTimeTextLine(nullptr), iConfirmEndTime(0),
				 iVisibleStartLine(0), iVisibleLineCount(0), iAdjustLine(0), DiffuseColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f)), lWaitingTime(0), iRestrictedCharacterCount(0), nAnswer(0), isTextCenterMode(0), isWaitFlag(0), poEventHandler(nullptr)
			{
