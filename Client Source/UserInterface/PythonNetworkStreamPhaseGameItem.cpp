...
bool CPythonNetworkStream::RecvItemGroundAddPacket()
{
	...
	CPythonItem::Instance().CreateItem(packet_item_ground_add.dwVID, 
									   ...
									   packet_item_ground_add.dwCount,
...
bool CPythonNetworkStream::SendShopSellPacketNew(WORD wSlot, WORD wCount)
{
	if (!__CanActMainInstance())
		return true;
	...
	Tracef(" SendShopSellPacketNew(wSlot=%d, wCount=%d)\n", wSlot, wCount);
...
bool CPythonNetworkStream::RecvAccePacket()
{
	...
	switch (AccePacket.subheader)
	{
		...
		{
			TItemPos itemPos;
			if (!Recv(sizeof(itemPos), &itemPos))
			{
				return false;
			}
