...
{
	auto it = m_map_pkParty.find(ch->GetPlayerID());
...
{
	auto it = m_memberMap.find(dwPID);
...
void CParty::Update()
{
	...
	if (!m_iLongTimeExpBonus && (get_dword_time() - m_dwPartyStartTime > PARTY_ENOUGH_MINUTE_FOR_EXP_BONUS * 30 * 1000))
	{
