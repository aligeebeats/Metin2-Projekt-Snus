...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		sys_log(1, "Shop::Buy : Not enough money : %s has %d, price %lld", ch->GetName(), ch->GetGold(), lldPrice);
		return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
	}
	...
	if (m_pkPC)
	{
		...
		m_pkPC->PointChange(POINT_GOLD, lldPrice, false);

		if (iVal > 0)
			m_pkPC->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d%% sales tax deducted."), iVal);
	}
	else
	{
		if (item->IsDragonSoul())
	...
	if (item)
		sys_log(0, "SHOP: BUY: name %s %s(x %d):%u price %u", ch->GetName(), item->GetName(), item->GetCount(), item->GetID(), lldPrice);
