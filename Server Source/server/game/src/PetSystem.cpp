...
void CPetSystem::HandlePetCostumeItem()
{
	...
	const CMob* mob = CMobManager::instance().Get(dwPetVnum);
	if (!mob)
	{
		return;
	}

	char nameBuf[CHARACTER_NAME_MAX_LEN];
	sprintf(nameBuf, "%s", m_pkOwner->GetName());

	Summon(dwPetVnum, pkPetCostume, nameBuf, false);
}
