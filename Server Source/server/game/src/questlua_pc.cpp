...
namespace quest 
{
	...
	int pc_give_gold(lua_State* L)
	{
		...
		long long lldAmount = (long long)lua_tonumber(L, 1);
		...
		ch->PointChange(POINT_GOLD, lldAmount, true);
		return 0;
	}
	...
	int pc_mount(lua_State* L)
	{
		...
		{
#ifdef ENABLE_SPECIAL_INVENTORY
			if (ch->GetEmptyInventory(armor) == -1)
#else
...
teleport_area:
	...
	int pc_acce_open_combine(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch) {
			sys_err("No character on a pc function! pc_acce_open_combine");
			return 0;
		}
	...
	int pc_acce_open_absorb(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch) {
			sys_err("No character on a pc function! pc_acce_open_absorb");
			return 0;
		}
