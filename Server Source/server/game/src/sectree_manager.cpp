...
{
	LPSECTREE tree = this->Get(lMapIndex, x, y);
...
bool SECTREE_MANAGER::GetMovablePosition(long lMapIndex, long x, long y, PIXEL_POSITION & pos)
{
	...
	do
	{
		...
		LPSECTREE tree = this->Get(lMapIndex, dx, dy);
...
{
	LPSECTREE_MAP pMap = this->GetMap(lMapIndex);
...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent)
	{
		if ( ent->IsType(ENTITY_CHARACTER) == true )
		{
			...
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsBuffNPC())
#else
...
{
	LPSECTREE_MAP sectree = this->GetMap(lMapIndex);
