...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, WORD wItemCount)
{
	...
	long long lldTotalMoney = 0;

	for (int n = 0; n < wItemCount; ++n)
	{
		lldTotalMoney += (pTable + n)->price;
	}

	lldTotalMoney += GetGold();
	...
	std::map<DWORD, long long> itemkind;
	...
	for (WORD i = 0; i < wItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot sell items you have locked."));
				return;
			}
...
#else
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC|| GetCharType() == CHAR_TYPE_MOUNT|| GetCharType() == CHAR_TYPE_PET)
#endif
	{
		...
		addPacket.dwPart[CHR_EQUIPPART_ARMOR] = GetPart(PART_MAIN);
		addPacket.dwPart[CHR_EQUIPPART_WEAPON] = GetPart(PART_WEAPON);
		addPacket.dwPart[CHR_EQUIPPART_HEAD] = GetPart(PART_HEAD);
		addPacket.dwPart[CHR_EQUIPPART_HAIR] = GetPart(PART_HAIR);
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
		if (IsBuffNPC() == true) {
			addPacket.dwPart[CHR_EQUIPPART_WEAPON] = GetBuffWearWeapon();
			addPacket.dwPart[CHR_EQUIPPART_HAIR] = GetBuffWearHead();
			addPacket.dwPart[CHR_EQUIPPART_ARMOR] = GetBuffWearArmor();
		}
#endif
		addPacket.dwPart[CHR_EQUIPPART_ACCE] = GetPart(PART_ACCE);

		LPITEM acce;
		if ((acce = GetWear(WEAR_COSTUME_ACCE)))
		{
			addPacket.byAcceDrainRate = acce->GetSocket(0);
		}
		else
		{
			addPacket.byAcceDrainRate = 0;
		}
...
void CHARACTER::UpdatePacket()
{
	...
	pack.dwPart[CHR_EQUIPPART_ARMOR] = GetPart(PART_MAIN);
	pack.dwPart[CHR_EQUIPPART_WEAPON] = GetPart(PART_WEAPON);
	pack.dwPart[CHR_EQUIPPART_HEAD] = GetPart(PART_HEAD);
	pack.dwPart[CHR_EQUIPPART_HAIR] = GetPart(PART_HAIR);
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
	if (IsBuffNPC() == true) {
		pack.dwPart[CHR_EQUIPPART_WEAPON] = GetBuffWearWeapon();
		pack.dwPart[CHR_EQUIPPART_HAIR] = GetBuffWearHead();
		pack.dwPart[CHR_EQUIPPART_ARMOR] = GetBuffWearArmor();
	}
#endif
	pack.dwPart[CHR_EQUIPPART_ACCE] = GetPart(PART_ACCE);

	LPITEM acce;
	if ((acce = GetWear(WEAR_COSTUME_ACCE)))
	{
		pack.byAcceDrainRate = acce->GetSocket(0);
	}
	else
	{
		pack.byAcceDrainRate = 0;
	}
...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
	...
	TPlayerSkill skill;
	memset(&skill, 0, sizeof(TPlayerSkill));
	skill.bLevel = GetHorseLevel();
	skill.bMasterType = GetSkillGradeByLevel(skill.bLevel);

	tab.skills[SKILL_HORSE] = skill;
...
void CHARACTER::SetProto(const CMob * pkMob)
{
	...
	if (CMobManager::instance().IsCostumeMount(GetRaceNum()))
	{
		m_stateIdle.Set(this, &CHARACTER::BeginStateEmpty, &CHARACTER::StateHorse, &CHARACTER::EndStateEmpty);
		m_stateMove.Set(this, &CHARACTER::BeginStateEmpty, &CHARACTER::StateMove, &CHARACTER::EndStateEmpty);
		m_stateBattle.Set(this, &CHARACTER::BeginStateEmpty, &CHARACTER::StateHorse, &CHARACTER::EndStateEmpty);
...
void CHARACTER::ComputeBattlePoints()
{
	...
	else if (IsPC())
	{
		...
		for (int i = 0; i < WEAR_MAX_NUM; ++i)
		{
			...
			else if ((pkItem = GetWear(i)) && pkItem->GetType() == ITEM_COSTUME && pkItem->GetSubType() == COSTUME_ACCE && pkItem->GetSocket(1) > 0)
			{
				TItemTable* p = ITEM_MANAGER::instance().GetTable(pkItem->GetSocket(1));
				if (p && p->bType == ITEM_ARMOR && p->bSubType == ARMOR_BODY)
				{
					long drainPct = pkItem->GetSocket(0);
					if (drainPct != 0)
					{
						iArmor += (int)(float(p->alValues[1]) / 100.0f * float(drainPct));
						iArmor += (int)(2 * (float(p->alValues[5]) / 100.0f * float(drainPct)));
					}
				}
			}
		}
...
void CHARACTER::ComputePoints()
{
	...
	if (IsPC())
	{
		...
		SetPoint(POINT_ATT_SPEED, DEFAULT_ATTACK_SPEED);
...
EVENTFUNC(recovery_event)
{
	...
	if (!ch->IsPC())
	{
		...
		{
			ch->PointChange(POINT_HP, MAX(1, (static_cast<long long>(ch->GetMaxHP() * ch->GetMobTable().bRegenPercent)) / 100));
		}
...
long long CHARACTER::GetPoint(WORD type) const
{
...
void CHARACTER::SetPoint(WORD type, long long val)
{
	if (type >= POINT_MAX_NUM)
	{
		sys_err("Point type overflow (type %u)", type);
...
void CHARACTER::PointChange(BYTE type, long long amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
...
#ifdef ENABLE_WOLFMAN
			if (GetJob() == JOB_WOLFMAN && GetLevel() >= 5)
			{
				if (!GetSkillGroup())
				{
					ClearSkill();
		...
		case POINT_GOLD:
			{
			const long long lldTotalMoney = static_cast<long long>(GetGold()) + static_cast<long long>(amount);
...
{
	return (abs(leader->GetLevel() - quest->GetLevel()) <= PARTY_CAN_JOIN_MAX_LEVEL);
}
...
CHARACTER::PartyJoinErrCode CHARACTER::IsPartyJoinableMutableCondition(const LPCHARACTER pchLeader, const LPCHARACTER pchGuest)
{
	if (!CPartyManager::instance().IsEnablePCParty())
   	...
   	{
	   	if (pchLeader->GetParty()->GetMemberCount() == MAX_MEMBER_INTRO_PARTY)
		...
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			if (GetMyShop())
			{
				...
				else
				{
					...
					if ((GetExchange() || IsOpenSafebox() || IsCubeOpen() || IsAcceWindowOpen()))
					{
...
void CHARACTER::SetTarget(LPCHARACTER pkChrTarget)
{
	...
	if (m_pkChrTarget)
	{
		...
		{
			p.bHPPercent = MINMAX(0, (static_cast<long long>(m_pkChrTarget->GetHP() * 100LL)) / m_pkChrTarget->GetMaxHP(), 100);
...
void CHARACTER::BroadcastTargetPacket()
{
	...
	{
		p.bHPPercent = MINMAX(0, (static_cast<long long>(GetHP() * 100LL)) / GetMaxHP(), 100);
		p.iMinHP = GetHP();
		p.iMaxHP = GetMaxHP();
	}
...
void CHARACTER::LoadSafebox(int iSize, long long lldGold, int iItemCount, TPlayerItem * pItems)
{
	bool bLoaded = false;
	SetOpenSafebox(true);
	...
	if (!m_pkSafebox)
		m_pkSafebox = M2_NEW CSafebox(this, iSize, lldGold);
	else
...
long long CHARACTER::ComputeRefineFee(long long lldCost, int iMultiply) const
{
	CGuild* pGuild = GetRefineGuild();
	if (pGuild)
	{
		if (pGuild == GetGuild())
			return lldCost * iMultiply * 9 / 10;

		LPCHARACTER chRefineNPC = CHARACTER_MANAGER::instance().Find(m_dwRefineNPCVID);
		if (chRefineNPC && chRefineNPC->GetEmpire() != GetEmpire())
			return lldCost * iMultiply * 3;

		return lldCost * iMultiply;
	}
	else
		return lldCost;
}

void CHARACTER::PayRefineFee(long long lldTotalMoney)
{
	long long lldFee = lldTotalMoney / 10;
	CGuild* pGuild = GetRefineGuild();

	long long lldRemain = lldTotalMoney;

	if (pGuild)
	{
		if (pGuild != GetGuild())
		{
			pGuild->RequestDepositMoney(this, lldFee);
			lldRemain -= lldFee;
		}
	}

	PointChange(POINT_GOLD, -lldRemain);
}
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	{
		if (GetExchange() || GetMyShop() || IsOpenSafebox() || IsCubeOpen() || IsAcceWindowOpen())
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot go elsewhere, cannot quit while after a trade."));
...
EVENTFUNC(check_speedhack_event)
{
	...
	{
		LogManager::instance().HackLog(ch->GetAID(), ch->GetName(), "SPEEDHACK");
	}
	...
	if (m_iComboHackCount >= 10)
	{
		if (GetDesc())
		{
			if (GetDesc()->DelayedDisconnect(number(2, 7)))
			{
				...
				LogManager::instance().HackLog(GetAID(), GetName(), "COMBO");
			}
		}
	}
}
