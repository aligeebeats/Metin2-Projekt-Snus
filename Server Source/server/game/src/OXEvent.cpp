...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? LC_TEXT("Âü") : LC_TEXT("FALSE"));
		}
	}
...
EVENTFUNC(oxevent_timer)
{
	...
	{
		case 0:
			SendNoticeMap(LC_TEXT("The answer follows in 10 sec."), OXEVENT_MAP_INDEX, true);
			flag++;
		...
		case 1:
			...
			if (info->answer == true)
			{
				...
				SendNoticeMap(LC_TEXT("Yes"), OXEVENT_MAP_INDEX, true);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendNoticeMap(LC_TEXT("No"), OXEVENT_MAP_INDEX, true);
			}

			SendNoticeMap(LC_TEXT("After 5 seconds, the participants who gave a wrong answer have to leave."), OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap(LC_TEXT("If it's correct walk to O, if it's wrong walk to X"), OXEVENT_MAP_INDEX, true);
